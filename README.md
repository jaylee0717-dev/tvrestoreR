...
`tvRestoreR` is an R package for image restoration using total variation
(TV) regularization.  
...
problem:$$ \min_{u} \frac{\lambda}{2}\int_{\Omega} |\nabla u| +  \int_{\Omega \cap D} (u - f)^2 dx $$
...
- Rudin, L. I., Osher, S., & Fatemi, E. (1992). Nonlinear total
  ...
  Phenomena*, 60(1-4), 259-268.

### Algorithms (The Solvers)

- **Primal-Dual** (`method = "primal_dual"`): Chambolle, A., & Pock, T.
  (2011). A first-order primal-dual algorithm for convex problems with
  applications to imaging. *Journal of Mathematical Imaging and Vision*,
  40(1), 120-145.

- **Split-Bregman** (`method = "split_bregman"`): Goldstein, T., &
  Osher, S. (2009). The split Bregman method for L1-regularized
